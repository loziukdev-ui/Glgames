(() => {
    ...
    let gotoblock_gotoBlock = (targetBlock, noHeader = false, speed = 500, offsetTop = 0) => {
        ...
        if (targetBlockElement) {
            ...
            functions_FLS(`[gotoBlock]: +`);
        } else functions_FLS(`[gotoBlock]: -`);
    };
    ...
    (function() {
        ...
        const fieldIds = [ "name", "email", "subject", "phone", "message" ];
        function getFieldContainer(inputElement) {
            return inputElement.closest(".form-field");
        }
        fieldIds.forEach((id => {
            const input = document.getElementById(id);
            if (input) input.addEventListener("input", (function() {
                const container = getFieldContainer(this);
        ...
        function validateForm() {
            ...
            if (!name.value.trim()) {
                getFieldContainer(name).classList.add("error");
                isValid = false;
            }
            if (!email.value.trim() || !/^[^\s@]+@([^\s@]+\.)+[^\s@]+$/.test(email.value.trim())) {
                getFieldContainer(email).classList.add("error");
                isValid = false;
            }
            if (!subject.value.trim()) {
                getFieldContainer(subject).classList.add("error");
                isValid = false;
            }
            if (!phone.value.trim()) {
                getFieldContainer(phone).classList.add("error");
                isValid = false;
            }
            if (!message.value.trim()) {
                getFieldContainer(message).classList.add("error");
                isValid = false;
            }
            return isValid;
        }
        form.addEventListener("submit", (async e => {
            ...
            try {
                ...
                if (result.success) {
                    ...
                    statusDiv.textContent = "❌ Error: " + (result.message || "Could not send message.");
    ...
    (function() {
        ...
        function injectStyles() {
            if (document.getElementById("reveal-lines-styles")) return;
            var style = document.createElement("style");
            style.id = "reveal-lines-styles";
            style.textContent = ".reveal-line-wrapper{display:block;overflow:hidden;line-height:inherit}" + ".reveal-line{display:block;transform:translateY(100%);transition:transform var(--reveal-duration,0.7s) cubic-bezier(0.2,0.9,0.3,1);will-change:transform;line-height:inherit}" + ".reveal-line-visible{transform:translateY(0)!important}";
            document.head.appendChild(style);
        }
        function processContainer(container) {
            if (container.classList.contains("reveal-processed")) return;
            if (!container._revealOriginalHTML) container._revealOriginalHTML = container.innerHTML;
            var duration = parseFloat(container.dataset.revealDuration) || .7;
            var lineDelay = parseFloat(container.dataset.revealDelay) || .1;
            var startDelay = parseFloat(container.dataset.revealStartDelay) || 0;
            var once = container.dataset.revealOnce !== "false";
            new SplitType(container, {
                types: "lines",
                tagName: "span",
                lineClass: "reveal-line-temp"
            });
            var tempLines = container.querySelectorAll(".reveal-line-temp");
            for (var i = 0; i < tempLines.length; i++) {
                var line = tempLines[i];
                var wrapper = document.createElement("span");
                wrapper.className = "reveal-line-wrapper";
                line.classList.remove("reveal-line-temp");
            ...
            var observer = new IntersectionObserver((function(entries) {
                entries.forEach((function(entry) {
                    if (entry.isIntersecting) {
                        var wrappers = container.querySelectorAll(".reveal-line-wrapper");
                        setTimeout((function() {
                            for (var i = 0; i < wrappers.length; i++) (function(index) {
                                var wrapper = wrappers[index];
                                var line = wrapper.querySelector(".reveal-line");
                                if (!line) return;
                                if (!once) {
                                ...
                                setTimeout((function() {
                                    line.classList.add("reveal-line-visible");
                                }), index * lineDelay * 1e3);
                            })(i);
                        }), startDelay * 1e3);
                        ...
                        for (var i = 0; i < lines.length; i++) lines[i].classList.remove("reveal-line-visible");
                    }
                }));
            }), {
                threshold: .2,
            ...
            container.classList.add("reveal-processed");
        }
        function initRevealLines() {
            var elements = document.querySelectorAll('[data-reveal="lines"]:not(.reveal-processed)');
            if (elements.length === 0) return;
            injectStyles();
            for (var i = 0; i < elements.length; i++) processContainer(elements[i]);
        }
        window.revealLinesRefresh = function() {
            var containers = document.querySelectorAll('[data-reveal="lines"].reveal-processed');
            for (var i = 0; i < containers.length; i++) {
                var el = containers[i];
                if (el._revealOriginalHTML) {
                    el.innerHTML = el._revealOriginalHTML;
                    el.classList.remove("reveal-processed");
                }
            }
            initRevealLines();
        };
        function ready() {
            if (document.fonts && document.fonts.ready) document.fonts.ready.then(initRevealLines); else initRevealLines();
        }
        loadSplitType((function() {
